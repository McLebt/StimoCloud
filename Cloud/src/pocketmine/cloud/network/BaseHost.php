...
use pocketmine\cloud\network\protocol\DededePacket;
use pocketmine\cloud\network\protocol\ListServersPacket;
use pocketmine\cloud\network\protocol\LoginPacket;
use pocketmine\cloud\network\protocol\RequestPacket;
use pocketmine\cloud\network\protocol\StartPrivateServerPacket;
...
class BaseHost{
	 ...
	 * @return void
	 */
	public function handleStartPrivateServerPacket(InternetAddress $address, string $requestId, string $group = "", string $owner = ""): void{
		...
		if ($this->cloud->isTemplate($group)) {
			...
			for ($i = 0; $i < 1; $i++) {
				if ($packet->owner !== "") {
					$server = $template->createNewPrivateServer($packet->owner);
					if (!is_null($server)) {
						$server->startPServer();
					} else {
						$packet->status = RequestPacket::STATUS_ERROR;
					...
					$this->logger->warning("§cPlease proviade a P-Server Owner!");
				}
			}
		} else {
			$this->logger->warning("§cThis is not a Cloud-Group!");
			...
			while (!$len === false) {
				...
				if (!$len === false) {
					...
					if (!is_null($packet)) {
						...
						if ($packet instanceof DededePacket) {
						...
						if ($packet instanceof StartPrivateServerPacket) {
							$this->handleStartPrivateServerPacket($address, $packet->requestId, $packet->template, $packet->owner);
						}
